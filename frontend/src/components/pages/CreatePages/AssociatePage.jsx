...
const AssociatePage = () => {
    ...
    // const [assignMemberdata, setAssignMemberData] = useState([])
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if (OfficeC && image && selectedState) {
    ...
    const handleEdit = async (value) => {
        ...
        setId(value.Id)
        setData({
            Appliname: value.Appliname,
            user: user || 'NA',
            IntrodName: value.IntrodName,
            FaHuName: value.FaHuName,
            MName: value.MName,
            gender: value.gender,
            ...
            passNo: value.passNo,
            DLNo: value.DLNo, Icard: value.Icard,
            bankName: value.bankName, BankCode: value.BankCode, AccountNo: value.AccountNo, IFSC: value.IFSC,
            BankAddr: value.BankAddr, Mobile: value.Mobile, Address: value.Address, district: value.district,
            PinCode: value.PinCode, Landline: value.Landline, email: value.email,
            company: value.company, experience: value.experience, CompanyAdd: value.CompanyAdd,
            NName: value.NName, NAge: value.NAge, relation: value.relation, NomAddr: value.NomAddr,
            organization: value.organization, AreaOccu: value.AreaOccu, TSize: value.TSize, images: value.image,
        ...
        setIntrodCode({
            IntrodCode: value.IntrodCode
        })
        if (value.image) {
            setPreview(`/api/v1/getall-associate/${value.image}`);
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            if (IntrodCode && OfficeC && Id && image) {
                ...
                // Resetting states after successful update
                setId('');
                setPopUp(false);
                acessData();
                setData({});
    ...
    const handleDelete = async (value) => {
        ...
        try {
            const response = await axios.delete(`/api/v1/delete-associate/${image}/${Id}`);
            if (response.data.success) {
                showToast({
                    str: response.data.message,
                    time: 500,
    ...
    const searchData = async (e) => {
        ...
        try {
            if (accessmemberdata) {
                // Ensure accessmemberdata is sent as an object
                        ...
                        <form onSubmit={changeValue ? handleUpdate : handleSubmit} style={{ zIndex: '100' }} className='absolute w-full top-14'>
                            <div className='bg-white py-6 rounded flex flex-col  relative sm:mx-12 mx-3 overflow-y-scroll h-[75vh] shadow-current shadow-sm font-serif sm:px-4  gap-2 md:gap-6'>
                                ...
                                <div className='lg:flex lg:gap-4 lg:justify-between cust '>
                                    ...
                                    <div className='flex flex-col lg:w-[33vw] gap-3'>

                                        <div className='flex flex-col gap-5 shadow-lg p-5'>
                                            <p className='font-bold text-lg'>Personal Details</p>

                                            <div className='flex flex-col md:flex-row gap-1 md:gap-3 border-b-2 relative'>
                                            ...
                                            <div className='flex flex-col md:flex-row gap-1 md:gap-3 border-b-2 mt-5'>
