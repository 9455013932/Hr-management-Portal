...
const CustomerPage = () => {
    ...
    const handleEdit = async (value) => {
        ...
        setData({
            ...
            IncomeGroup: value.IncomeGroup,
            SO: value.SO,
            PropertyType: value.PropertyType,
            EMI: value.EMI,
            IntroName: value.IntroName,
            gender: value.gender,
            Category: value.Category,
            AgreeDate: value.AgreeDate,
            DOB: value.DOB,
            Nationality: value.Nationality,
            ProjectSize: value.ProjectSize,
            PayDate: value.PayDate,
            PAN: value.PAN,
            PayDMode: value.PayDMode,
            SupName: value.SupName,
            GaurdName: value.GaurdName,
            GaurdAge: value.GaurdAge,
            PayMode: value.PayMode,
            GuardAddr: value.GuardAddr,
            PayableAt: value.PayableAt,
            AmountWord: value.AmountWord,
            ...
            ProReference: value.ProReference,
            PLCCost: value.PLCCost,
            DeveCharge: value.DeveCharge,
            ProCost: value.ProCost,
            ProjectName: value.ProjectName,
            AreaOccu: value.AreaOccu,
            TSize: value.TSize,
            ...
            image: value.image,
            chequeImg: value.chequeImg,
            term: value.term
        });
        setIntroCode({ IntroCode: value.IntroCode })

        if (value.image || value.chequeImg) {
            setImage([value.image, value.chequeImg]);
            setPreview(`/api/v1/getall-customer/${value.image}`);
            setCPreview(`/api/v1/getall-customer/${value.chequeImg}`);
        } else {
            setPreview('');
            setCPreview('');
            setImage([]);
    ...
    const handleDelete = async (value) => {
        const { Id } = value;
        try {
            const response = await axios.delete(`/api/v1/delete-customer/${Id}`);
            if (response.data.success) {
                showToast({
                    str: response.data.message,
                    time: 500,
    ...
    const handleBlur = async () => {
        ...
        try {
            if (ProjectSize) {
                const { data } = await axios.post('/api/v1/get-plot-price', { ProjectSize })
                if (data.success) {
                    let projectPrice = parseInt(data.result[0].proprice)
            ...
            console.log('Plot amount not assign')
        }
    }
    ...
    const handlePayCharge = async () => {
        ...
        const bookingCharge = parseInt(data.BookAmount, 10);
                                ...
                                <div className='lg:flex lg:gap-4 lg:justify-between associate '>
                                    ...
                                    <div className='flex flex-col gap-2'>
                                        ...
                                        {/* <div className='flex-col gap-5 shadow-lg p-5 flex'>
                                            ...
                                            <div className='flex flex-col md:flex-row gap-1  border-b-2'>
                                                <label htmlFor="Description" className='whitespace-nowrap'>Project Description :</label>
                                                <input type="text" name="Description" id="Description" value={data.Description} onChange={handleChange} className='w-full outline-none px-2 py-1' />
                                            </div>

                                            <div className='flex flex-col md:flex-row gap-1 md:gap-3 text-[0.9rem]'>
                                                <div className='w-full flex gap-3'>
                                                    <input type="radio" name="DirectionE" id="" value='East' checked={data.DirectionE == 'East'} onChange={handleChange} className='text-[0.9rem] outline-none' />East
                                                    <input type="radio" name="DirectionE" id="" value='North-East' checked={data.DirectionE == 'North-East'} onChange={handleChange} className='text-[0.9rem] outline-none  ' />North-East
                                                </div>
                                            </div>

                                            <div className='flex flex-col md:flex-row gap-1 md:gap-3 text-[0.9rem]'>
                                                <div className='w-full flex gap-3'>
                                                    <input type="radio" name="DirectionW" id="" value='West' checked={data.DirectionW == 'West'} onChange={handleChange} className='text-[0.9rem] outline-none' />West
                                                    <input type="radio" name="DirectionW" id="" value='South-East' checked={data.DirectionW == 'South-East'} onChange={handleChange} className='text-[0.9rem] outline-none  ' />South-East
                                                </div>
                                            </div>

                                            <div className='flex flex-col md:flex-row gap-1 md:gap-3 text-[0.9rem] '>
                                                <div className='w-full flex gap-3'>
                                                    <input type="radio" name="DirectionN" id="" value='North' checked={data.DirectionN == 'North'} onChange={handleChange} className='text-[0.9rem] outline-none' />North
                                                    <input type="radio" name="DirectionN" id="" value='North-West' checked={data.DirectionN == 'North-West'} onChange={handleChange} className='text-[0.9rem] outline-none  ' />North-West
                                                </div>
                                            </div>

                                            <div className='flex flex-col md:flex-row gap-1 md:gap-3 text-[0.9rem] '>
                                                <div className='w-full flex gap-3'>
                                                    <input type="radio" name="DirectionS" id="" value='South' checked={data.DirectionS == 'South'} onChange={handleChange} className='text-[0.9rem] outline-none' />South
                                                    <input type="radio" name="DirectionS" id="" value='South-West' checked={data.DirectionS == 'South-West'} onChange={handleChange} className='text-[0.9rem] outline-none  ' />South-West
                                                </div>
                                            </div>
