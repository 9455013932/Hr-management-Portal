...
const EMIPayment = () => {
    ...
    useEffect(() => {
        if (allResult && data.ToInstall) {
            const value = allResult[0]
            let prevTerm = parseInt(value.ToInstall)
            // console.log('prevTerm', prevTerm)
            let EmiMonthly = parseInt(value.EMI)
            // console.log("EmiMonthly", EmiMonthly)
            let nextTerm = parseInt(data.ToInstall)
            // console.log("nextTerm", nextTerm)
            let nextEmi = nextTerm - prevTerm
            // console.log(nextEmi)
            let amountWord = toWords.convert(nextEmi * EmiMonthly)
            // console.log(amountWord)
            setData({
                ...data,
                PayAmount: nextEmi * EmiMonthly,
    ...
    const handleValues = async (e) => {
        ...
        try {
            if (value) {
                const { data } = await axios.post(`/api/v1/get-customer-id/${value}`)
                if (data.success) {
    ...
    const handleClick = async (e) => {
        ...
        if (allResult) {
            ...
            const expiry = value.AgreeDate
            let date = new Date(expiry)
            let termParts = value.term.split('.')
            // console.log(date,term)

            const years = parseInt(termParts[0], 10)
            const months = termParts[1] ? Math.round(parseFloat(`0.${termParts[1]}`) * 12) : 0
            date.setFullYear(date.getFullYear() + years)
            date.setMonth(date.getMonth() + months)
            const timeDate = date.toISOString()

            // next due date
            let newDate = new Date(timeDate)
            newDate.setDate(newDate.getDate() + 1)
            const dueDate = newDate.toISOString()

            let nextInstall = []
            for (let i = parseInt(value.ToInstall) + 1; i < 46; i++) {
                nextInstall.push(i)
            }
            setToInstall(nextInstall)

            let prevTerm = parseInt(value.ToInstall) + 1
            setTimeout(() => {
                setData({
                    ...data,
                    ...
                    ExpireDate: timeDate.split('T')[0],
                    DueDate: dueDate.split('T')[0],
                    P_Date: value.PayDate,
                    FromInstall: prevTerm,
                    ToInstall: prevTerm,
                    PayAmount: value.EMI
                })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('/api/v1/create-branch-emipayment', formData, {
                headers: {
        ...
        <BranchLayout>
            <div className='w-full h-full bg-white shadow-2xl flex flex-col gap-5  relative'>
                <div className='h-14 flex  items-center sm:mx-12 mx-5 justify-between shadow  md:px-3 px-1'>
                    <p className='md:text-xl text-gray-400 font-semibold'>EMI Payment</p>
                ...
                {popUp && (
                    <>
                        <form target='_blank' onSubmit={changeValue ? handleUpdate : handleSubmit} style={{ zIndex: '100' }} className='absolute w-full top-14 '>
                            <div className='bg-white py-6 rounded flex flex-col  relative sm:mx-12 mx-3 overflow-y-scroll h-[75vh] scrollbar-hide shadow-current shadow-sm font-serif sm:px-4 px-2 gap-2 md:gap-6'>
                                ...
                                <div className='lg:flex lg:gap-4 lg:justify-between associate mx-auto'>
                                    ...
                                    <div className='flex flex-col gap-2'>
                                        ...
                                        <div className='flex-col gap-5 shadow-lg p-5 flex'>
                                            ...
                                            <div className='flex flex-col md:flex-row gap-1  border-b-2'>
                                                <label htmlFor="ExpireDate" className='whitespace-nowrap'>Reciept's Expiry Date :</label>
                                                <input type="date" name="ExpireDate" id="ExpireDate"
                                                    value={data.ExpireDate} readOnly onChange={handleChange} className='w-full outline-none px-2 py-1' />
                                            </div>

                                            <div className='flex flex-col md:flex-row gap-1  border-b-2'>
                                                <label htmlFor="DueDate" className='whitespace-nowrap'>Next Due Date :</label>
                                                <input type="date" name="DueDate" id="DueDate"
                                                    value={data.DueDate} readOnly onChange={handleChange} className='w-full outline-none px-2 py-1' />
                                            </div>
                                            ...
                                            <div className='flex flex-col md:flex-row gap-1  border-b-2'>
                                                <label htmlFor="FromInstall" className='whitespace-nowrap'>From Installment :</label>
                                                <input type="text" name="FromInstall" id="FromInstall"
                                                    value={data.FromInstall} readOnly onChange={handleChange} className='w-full outline-none px-2 py-1' />
                                            </div>

                                            <div className='flex flex-col md:flex-row gap-1  border-b-2'>
                                                <label htmlFor="TransBank" className='whitespace-nowrap'>To Installment :</label>
                                                <select className='w-full outline-none px-2 py-1' value={data.ToInstall} required onChange={(e) => { handleChange(e); handleTerm(e) }} name="ToInstall" id="ToInstall" >
                                                    {
                                                        toInstall && toInstall.map((item, index) => (
                                    ...
                                    <div className='flex flex-col gap-5'>
                                        ...
                                        <div className='flex flex-col gap-5'>
                                            ...
                                            <div className='flex flex-col md:flex-row gap-1  border-b-2'>
                                                ...
                                                <input type='date' name="P_Date" id="P_Date" value={data.P_Date} readOnly onChange={handleChange} className='w-full outline-none px-2 py-1 ' />
                                            </div>

                                            <div className='flex flex-col md:flex-row gap-1  border-b-2'>
                                                <label htmlFor="ReliefAmount" className='whitespace-nowrap'>Relief(Rs.) :</label>
                                                <input type="text" name="ReliefAmount" id="ReliefAmount" value={data.ReliefAmount} onChange={handleChange} className='w-full outline-none px-2 py-1' />
                                            </div>
